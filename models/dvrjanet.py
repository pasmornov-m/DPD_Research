...
class DVRJANET(BaseModel, torch.nn.Module):
    def __init__(self, hidden_size, output_size, num_dvr_units=4, bias=True):
        ...
        self.W_ph = torch.nn.Linear(hidden_size, hidden_size, bias=False)
        self.W_pθ = torch.nn.Linear(1, hidden_size, bias=False)

        # Magnitude Recurrent Filter
        self.W_ah = torch.nn.Linear(hidden_size, hidden_size, bias=False)
        self.W_ax = torch.nn.Linear(1, hidden_size, bias=False)
        # DVR parameters
        self.cs = torch.nn.Parameter(torch.randn(num_dvr_units))

        # Processing gates
        self.W_f = torch.nn.Linear(hidden_size, hidden_size, bias=bias)
        self.W_ccos = torch.nn.Linear(hidden_size + hidden_size, hidden_size, bias=bias)
        self.W_csin = torch.nn.Linear(hidden_size + hidden_size, hidden_size, bias=bias)
